...
import { UploadStage, useUpload, UseUploadOptions } from "@/modules/shared/hooks/useImageUpload";
...
export function FileUploader({
  ...
  const busy = ["compressing", "signing", "uploading", "verifying"].includes(stage);
  ...
  if (children) {
    return (
      <>
        {children({ open: () => inputRef.current?.click(), stage, progress })}
        <input
          ref={inputRef}
          type="file"
          accept={accept.join(",")}
    ...
    <div className={cn("space-y-2", className)}>
      ...
      <div
        ...
        className={cn(
          ...
          stage === "error" && "border-destructive/40 bg-destructive/5",
      ...
      >
        ...
        <div className="text-center space-y-1">
          <p className={cn(
            "text-sm font-medium",
            stage === "done"  ? "text-green-600 dark:text-green-400" :
          ...
          {stage === "idle" && (
            <p className="text-xs text-muted-foreground">
              {hint ?? `${accept.map((a) => a.split("/")[1].toUpperCase()).join(", ")} · max ${Math.round(maxBytes / 1024 / 1024)} MB`}
            </p>
          )}
          {stage === "uploading" && (
