...
import { suspendUserServer } from "@/modules/admin/users/server/admin.users.server";
...
export async function PATCH(
  ...
  try {
    ...
    await suspendUserServer(id, parsed.data.reason);
    return new NextResponse(null, { status: 204 });
  } catch (error) {
    return handleServerError(error, "Failed to suspend user");
  }
}